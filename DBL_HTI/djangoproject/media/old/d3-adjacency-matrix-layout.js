...
function groupBy(objectArray, property) {
  ...
  function d3AdjacencyMatrixLayout () {
    ...
    var edgeWeight = function edgeWeight(d) {
      return d.value; //can we please hire a hitman for the person who made this? <3 <3 <3
    };
    ...
    function matrix() {
      ...
      edges.forEach(function (edge) {
        ...
        if (typeof edge.source === 'number') {
          constructedEdge.source = nodes.find(x => x.id == edge.source);
        }
        if (typeof edge.target === 'number') {
          constructedEdge.target = nodes.find(x => x.id == edge.target);
        }
        ...
        if (!edgeHash[id]) {
          // console.log(id, constructedEdge);
          edgeHash[id] = constructedEdge; // here are all the nodes that have an actuall weight
        } else {
          // isn't called????
          alert("asdsd");
